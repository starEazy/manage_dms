    ...
    if (typeof bearerHeader != 'undefined') {
      try {
        ...
        if (decode == 'expired') {
          ...
          const query = `SELECT * from Sec_UserSessionLog where userid = ${user_id} AND isasctive = true AND token = '${bearerHeader}'`
      ...
      try {
        ...
        if (decode == 'expired') {
          ...
          if (data.length > 0) {
            ...
            return unauthorizedResponse(req, res, `DMS Token Not Authenticated`)
          }
        }
      } catch (e) {
        console.log(e)
